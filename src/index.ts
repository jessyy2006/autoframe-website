import { RainbowAutoFramingConfig, RainbowAutoFramingLibrary } from "rainbow-auto-framing";
...
const framedVideo: HTMLVideoElement = document.getElementById("framedVideo") as HTMLVideoElement;
	...
	navigator.mediaDevices
		...
		.then(function (stream) {
			...
			originalVideo.addEventListener("loadeddata", async (event) => {
				const framedStream = library.autoframe(stream);

				framedVideo.width = 640; //width;
				framedVideo.height = 480; //height;

				framedVideo.srcObject = framedStream;

				console.log(`framedVideo w=${framedVideo.videoWidth}, h=${framedVideo.videoHeight}`);
				// ctx.drawImage isn't working, params are whack
				
				framedVideo.play().catch((e) => { console.warn("Video play failed:", e); });
			});
		})
		.catch((err) => {
			console.error(err);
...
async function loadConfig(config_path: string): Promise<RainbowAutoFramingConfig> {
	...
	try {
		...
		if (!response.ok) throw new Error(`HTTP error! status: ${response.status} while fetching config.json`);
